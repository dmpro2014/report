...
These modes introduce significant energy savings, allowing the CPU to spend most of its time asleep.
...
Let's say we're targeting 30FPS on the tunnel kernel.
The kernel itself takes 3 ms to complete @ 64 x 64 resolution.
Kernel launch overhead is 2~ms.
This allows the CPU to spend 60\% of its time in EM1 or 2, netting an average of (NUMBER * 0.40 + OTHERNUMBER * 0.60) power consumption.

For the throughput-intensive kernel, say the kernel itself takes 20ms to complete.
Kernel launch overhead is 5ms, resulting in a 80\% idle time that can be spent sleeping.
This averages to a power consumption of NUMBER.

While the CPU can save power easily, it is more difficult for the GPU on the FPGA.
Luckily Xilinx has recently implemented dynamic clock gating for the Spartan-6 series.
This allows for a reduction up to 30\% of dynamic power consumption, introducing significant energy savings.
This was not implemented for this project however, mainly due to time concerns.
