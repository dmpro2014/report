...
Screen tearing is a visual artefact where parts of two consecutive frames are displayed at the same time.
This occurs because the video unit reads the frame buffer before the GPU has finished rendering it.  
In figure \ref{fig:single_buffering} the artefact can be observed, occurrences are marked with red circles.
...
\end{figure}
Double buffering is a technique to remove this artefact.
...
The image in the figure does have some artefacts, but the ones caused by single buffering are no longer present.
\begin{figure}[H]
	\centering
