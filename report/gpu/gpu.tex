...
\begin{figure}[H]
	...
	\caption{A register directory containing the register files for a processor core.}
...
The register files are identical to each other, and may be select by using the \textit{Register file select signal}.
...
Instead it's left as a responsibility for the programmer to avoid using the \textit{LSU data} register when the LSU needs it.
