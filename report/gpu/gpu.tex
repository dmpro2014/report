...
The CPU issues commands to the command unit in the GPU. Commands are launching a kernel, uploading kernels to the instruction memory, writing to the constant memory, and read/write to SRAM.
...
Upon receiving a \emph{finished} instruction the control unit asserts the finished signal alerting the thread spawner that a new warp has to be spawned.
