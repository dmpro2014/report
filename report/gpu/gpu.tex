...
\begin{figure}[H]
\centering
\includegraphics[scale=0.2]{gpu/diagrams/architecture_overview.png}
\caption{A high level overview of the GPU.}
\label{architecture_overview}
\end{figure}
Figure \ref{architecture_overview} presents a high level overview over the data-flow in the GPU.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{gpu/diagrams/kernel_run_state_machine.png}
	\caption{The GPU's internal state during kernel execution.}
	\label{kernel_run_state_machine}
\end{figure}
A normal kernel execution can be represented by the state machine in figure \ref{kernel_run_state_machine}.
...
The thread spawner keeps track of the number of threads awaiting launch, when the thread spawner receives a \textit{Finished} instruction and no more threads are awaiting launch the kernel run
has completed.
